...
@php
    ...
    $accordionId = $id ?? 'accordion-item-' . uniqid();
    ...
    <h2>
        <button
            ...
            @click="
                ...
                if (open) {
                    activeItems = ['{{ $accordionId }}'];
                }
                @else
                if (open) {
                    if (!activeItems.includes('{{ $accordionId }}')) {
                        activeItems.push('{{ $accordionId }}');
                    }
                } else {
                    activeItems = activeItems.filter(item => item !== '{{ $accordionId }}');
                }
                @endif
            "
