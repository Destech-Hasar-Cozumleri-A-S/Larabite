{{-- Clipboard Input Group with Label --}}
@props([
    'label' => null,
    'prefix' => null, // Prefix text or icon
    'value' => '',
...
<div {{ $attributes->only(['class', 'wire:key']) }}>
    @if($label)
        <label for="{{ $targetId }}" class="block mb-2 {{ $sizeClasses[$size] }} font-medium text-gray-900 dark:text-white">
            {{ $label }}
        </label>
    @endif
    ...
    <div class="flex">
        @if($prefix)
            <span class="inline-flex items-center {{ $prefixClasses[$size] }} font-medium text-gray-900 bg-gray-200 border border-e-0 border-gray-300 rounded-s-lg dark:bg-gray-600 dark:text-gray-400 dark:border-gray-600">
                {!! $prefix !!}
            </span>
        @endif
        ...
        <div class="relative w-full">
            <input
                id="{{ $targetId }}"
                type="text"
                class="rounded-none {{ $prefix ? 'rounded-e-lg' : 'rounded-lg' }} bg-gray-50 border border-gray-300 text-gray-900 focus:ring-blue-500 focus:border-blue-500 block flex-1 min-w-0 w-full {{ $inputSizeClasses[$size] }} pe-11 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 {{ $disabled ? 'cursor-not-allowed opacity-60' : '' }}"
                value="{{ $value }}"
            ...
            <button
                ...
                @endif
                class="absolute end-2 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg {{ $buttonSizeClasses[$size] }} inline-flex items-center justify-center {{ $disabled ? 'cursor-not-allowed opacity-60' : '' }}"
                type="button"
                {{ $disabled ? 'disabled' : '' }}
            >
