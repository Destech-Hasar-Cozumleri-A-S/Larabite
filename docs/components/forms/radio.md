...
Radio button component - allows users to select a single option from multiple options.
...
- 5 variant (default, inline, list-group, bordered, color)
- 7 color option (blue, red, green, purple, teal, yellow, orange)
- Helper text support
- Description/subtitle support (for bordered variant)
- Icon support (for bordered variant)
...
| `name` | string | **required** | Radio button group name |
| `value` | mixed | **required** | Radio button value |
| `checked` | boolean | false | Initially selected or not |
| `disabled` | boolean | false | Disabled state |
| `required` | boolean | false | Required field indicator |
| `error` | string | optional | Error message |
| `success` | string | optional | Success message |
| `helper` | string | optional | Helper text |
| `variant` | string | 'default' | Variant: default, inline, list-group, bordered, color |
...
| `icon` | slot | optional | Icon content (for bordered variant) |
...
### List Group
For use in list format

### Bordered
Card style, large clickable area
...
- Radio buttons must be grouped with the same `name`
- Only one radio button can be selected in a group
- Each radio button requires a unique `value`
- For accessibility, each radio button must have a label
- List group variant must be used in a `ul` wrapper
- Bordered variant provides a larger clickable area (label covers the entire card)
- 7 different color options can be used with the color variant
- SVG icons can be added using the icon slot (for bordered variant)
- `wire:model` or `wire:model.live` can be used with Livewire
- `wire:model.live` captures changes instantly (ideal for real-time validation)
