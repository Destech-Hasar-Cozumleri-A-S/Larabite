...
Toggle/Switch component - for two-state (on/off, active/inactive) selections.
...
- 4 variants (default, double-label, icon, card)
- 2 size options (base, lg)
- 7 color options (blue, red, green, purple, teal, yellow, orange)
- Description support (for card variant)
- Icon support (for card variant)
...
| `name` | string | **required** | Toggle input name |
| `value` | string | '1' | Toggle value |
| `checked` | boolean | false | Initially checked state |
| `disabled` | boolean | false | Disabled state |
| `required` | boolean | false | Required field indicator |
| `error` | string | optional | Error message |
| `success` | string | optional | Success message |
| `helper` | string | optional | Helper text |
| `size` | string | 'base' | Toggle size: base, lg |
| `color` | string | 'blue' | Color: blue, red, green, purple, teal, yellow, orange |
| `variant` | string | 'default' | Variant: default, double-label, icon, card |
| `labelBefore` | string | optional | Label before toggle (for double-label variant) |
| `labelAfter` | string | optional | Label after toggle (for double-label variant) |
| `description` | string | optional | Description text (for card variant) |
| `icon` | slot | optional | Icon content (for card variant) |

## Variants

### Default
Standard toggle (label on the right)

### Double-Label
Two-sided label

### Icon
With check/X icons inside
...
- Toggle is implemented as checkbox input
- State management is done with CSS `peer` selector
- RTL (Right-to-Left) language support is available
- Keyboard navigation is supported with focus state
- Modern appearance with smooth transition animations
- Card variant provides more description and visual richness
- Icon variant automatically adds default check/X icons
- Double-label variant is ideal for explicit states like On/Off, Yes/No
- Can be used with Livewire `wire:model` or `wire:model.live`
- `wire:model.live` captures changes instantly (for real-time updates)
- User interaction can be prevented with disabled state
