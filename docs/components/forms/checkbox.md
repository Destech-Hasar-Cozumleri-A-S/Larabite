...
Checkbox (onay kutusu) seçimleri için bileşen.
...
- 6 varyant (default, link, bordered, list-group, inline, color)
- 7 renk seçeneği (blue, red, green, purple, teal, yellow, orange)
- Helper text desteği
- Description/açıklama desteği
- Icon desteği (bordered varyant için)
...
|------|------|---------|-------------|
| `label` | string | optional | Label metni (slot olarak da gönderilebilir) |
| `name` | string | optional | Checkbox name attribute |
| `value` | string | optional | Checkbox value |
| `checked` | boolean | false | Varsayılan olarak işaretli mi? |
...
| `variant` | string | 'default' | Varyant: default, link, bordered, list-group, inline, color |
| `color` | string | 'blue' | Renk: blue, red, green, purple, teal, yellow, orange |
| `description` | string | optional | Açıklama metni (bordered varyantı için) |
| `icon` | slot | optional | Icon (bordered varyantı için) |
...
| `description` | string | optional | Ek açıklama |
...
### Link
Label içinde link elementleri içerebilir

### Bordered
Kart tarzı border ile, daha geniş tıklama alanı

### List Group
Liste grubunda kullanım için (ul > li içinde)

### Inline
Yatay dizilim için

### Color
Renkli checkbox varyantları
...
- Multiple selection için `name` array olmalıdır (örn: `name="items[]"`)
- Helper text için `aria-describedby` accessibility attribute'u otomatik eklenir
- Bordered varyantı daha büyük tıklama alanı sağlar (kart boyutunda)
- List group varyantı `ul` wrapper içinde kullanılmalıdır
- Color varyantı ile 7 farklı renk seçeneği kullanılabilir
- Label slot kullanılarak HTML içerik eklenebilir (linkler için)
